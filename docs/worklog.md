...
- **프롬프트 9(계정 시스템) 관련해서 아직 없는 env 값들** - 위 "오늘 인수인계
  요약"의 콘솔 설정 3가지가 끝나야 채울 수 있음. 값이 없어도 나머지 기능(보드/
  커뮤니티/랭킹/담아가기)은 그대로 잘 동작하니 개발 자체를 막진 않음:
  - `FIREBASE_WEB_API_KEY` (이메일 로그인/회원가입)
  - `KAKAO_CLIENT_SECRET` (카카오 로그인 - `KAKAO_REST_API_KEY`는 기존 지도
    API용으로 이미 있음, 로그인엔 이 Secret이 추가로 필요)
  - `NAVER_CLIENT_ID` / `NAVER_CLIENT_SECRET` (네이버 로그인 - 완전히 새 앱)
