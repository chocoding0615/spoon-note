...
## 오늘 인수인계 요약 (2026-08-27 밤~28, 집 → 사무실)

어제(2026-08-27) 낮 사무실→집 인수인계(계정 시스템 콘솔 설정) 이후, 집에서
프롬프트 10·11 구현 + 실사용 중 발견된 버그 여러 개를 고쳤음. 전부
`chocoding0615/spoon-note` main에 푸시 완료(최신 커밋 `f100c55`), Vercel
프로덕션에도 반영돼있음. **콘솔 설정 3가지(Firebase/카카오/네이버)는 전부
끝났고 실제 로그인 세 경로 다 성공 확인함** - 그 부분은 더 볼 것 없음(아래
"2026-08-27 (오후)" 섹션에 완료 기록만 남아있음).

**완료됨(코드+배포+검증 전부 끝)**
- 프롬프트 10: 하단 탭 3개(홈/커뮤니티/마이페이지), 커뮤니티 피드 정렬 4종
  (인기·거리·최신·담아간 횟수순), 홈 탭 대시보드(지역선택+현재위치, 랭킹
  TOP5, 이번 주 급상승, 검색, 지도 진입점)
- 프롬프트 11: `/map` - 지역별 찜 합계를 버블로, 확대하면 개별 장소 마커로
  전환. `regionStats` 캐시 컬렉션을 새로 만들어서 지도 로드마다
  canonicalPlaces 전체를 재집계하지 않게 함
- 무료 보드 한도 3개 → **10개**로 확대(사용자 요청), 관련 시간당 보드 생성
  제한도 10 → 20회로 같이 조정
- 보드 상세 화면에 소유자 전용 **"설정" 버튼** 추가 - 제목/설명/공개설정
  수정 + 삭제(전엔 이 UI 자체가 없었음)

**오늘 실사용 중 발견해서 고친 버그 4개 - 사무실에서 비슷한 거 또 만들 때 참고**
1. **네이버 폴더 가져오기가 장소명을 영문/로마자로 가져옴** - 원인:
   `pages.map.naver.com`의 비공식 북마크 API가 `accept-language` 헤더로
   응답 언어를 정하는데 Node `fetch`(undici)는 이 헤더를 기본으로 안 보냄
   (curl은 되고 Node fetch는 안 되는 식이라 처음엔 "된다"고 착각하기 쉬움).
   **교훈: 외부 API 호출은 curl로만 확인하지 말고 실제 런타임(Node fetch)으로도
   확인할 것 - 클라이언트 라이브러리마다 기본 헤더가 다를 수 있음.**
2. **`PasteBox`는 `boardSlug`가 있어야만 폴더 링크를 인식한다** - 마이페이지의
   "링크로 보드 만들기" 자리에 `boardSlug` 없이 `<PasteBox/>`만 놔뒀더니(원래
   홈페이지 시절부터 쭉 그랬음) 미리보기만 뜨고 실제로는 아무것도 저장 안 됐음.
   **교훈: `PasteBox`를 "아직 보드가 없는" 화면에 쓸 땐 반드시
   `CreateBoardPasteBox`(먼저 보드를 만들고 그 위에서 기존 파이프라인을
   태우는 래퍼, `components/paste/CreateBoardPasteBox.tsx`)를 대신 쓸 것 -
   맨 `PasteBox`는 그 자리에서 아무 것도 안 만듦.**
3. **fetch 응답의 상태코드(res.ok)를 확인 안 하면 "실패"와 "정상인데 결과가
   아니다"를 구분 못 함** - `import-list`/`parse` 응답이 429(레이트리밋)나
   500이어도 `isPlacelist`/`parsed` 필드가 없으면 그냥 "지원 안 하는 링크"
   (정상 케이스)와 똑같이 취급돼서 조용히 빈 보드로 넘어갔음.
   **교훈: fetch 결과를 분기할 때 필드 유무보다 먼저 `res.ok`부터 확인할 것.**
4. **단축링크(naver.me 등) 리다이렉트 추적은 일시적으로 실패할 수 있고, 그
   실패가 "이 링크는 지원 안 함"과 똑같은 모양(200 응답, `isPlacelist:false`)
   으로 나온다** - res.ok 체크로도 못 잡는 종류. 완전히 못 없애는 문제(외부
   서비스 의존)라 `detectPlacelist`/파서/`parseUrl` 호출에 1회 재시도만
   추가해뒀음(`app/api/boards/[slug]/import-list/route.ts`,
   `app/api/parse/route.ts`). **또 재현되면 Vercel 로그에
   "[import-list] 폴더 링크 판별 실패" 로그가 남으니 확인할 것 - 재시도까지
   실패한 진짜 케이스만 로그 남게 해놨음.**

**환경 관련 알아둘 점**
- 레이트리밋(`lib/rateLimit.ts`)은 Firestore 기반, `x-forwarded-for`/
  `x-real-ip` 헤더로 IP를 얻는다. **로컬 dev 서버에 curl/브라우저로 테스트하면
  이 헤더가 없어서 항상 fail-open(무제한 통과)이라 레이트리밋 관련 버그는
  로컬에서 절대 재현 안 됨** - 꼭 실제 배포(Vercel) URL로 테스트할 것.
- Vercel은 지금 무료 Hobby 플랜. 사용자가 Pro 업그레이드($20~/월, 함수 실행
  시간·트래픽 한도 완화)는 **차후로 미룸** - 코드는 이미 Hobby에서도 안전하게
  동작하도록 짜놔서(예: `import-list` 라우트 `maxDuration=30`) 지금 당장
  뭘 더 할 필요는 없음. 업그레이드하면 알려달라고 해뒀음(그때 `maxDuration`
  더 올려줄 수 있음).

**여전히 미룬 것(전에도 미뤘고 이번에도 안 건드림)**
- 구글 로그인: 생년월일 scope는 OAuth 앱 심사가 필요해서 카카오/네이버/이메일
  먼저 끝내기로 하고 보류 중 - 재개 안 됨
- 카카오 "생년" 동의항목: 사업자 등록이 있어야 콘솔에서 켤 수 있어서 여전히
  꺼둔 상태(현재 카카오 로그인은 닉네임/프로필 사진만 받음, `birthYear`는 항상
  null → 연령 미상으로 안전 우선 처리됨)
...
## 버그 수정 — 링크 인식이 가끔 조용히 실패하는 문제(재시도 추가) (2026-08-28)

같은 네이버 폴더 링크로 사용자가 다시 테스트했는데 또 빈 보드("ff"라는 제목으로
직접 리네임한 것 - 원래는 CreateBoardPasteBox의 placeholder "새 보드")가 나옴.
Firestore에서 그 보드의 생성 시각을 직접 조회해보니 바로 전 커밋(res.ok 체크 fix,
23:45)보다 4분 뒤(23:49)에 만들어졌음 - 즉 그 수정이 이미 배포된 상태에서도
재현됨. 로컬/프로덕션 양쪽에서 같은 링크로 6번 넘게 반복 테스트했는데 전부
성공(3.7~5.9초 소요, 27개 정상)해서 "항상 실패"하는 버그는 아니고, 그 순간에만
실패한 **일시적** 문제로 보임.

- **원인 추정**: `naver.me` 같은 단축링크는 `resolveFinalUrl`/`detectPlacelist`가
  리다이렉트를 여러 홉 따라가야 하는데, 이 외부 왕복(네이버 서버 응답 지연,
  일시적 네트워크 오류 등)이 실패하면 `detectPlacelist`가 null을 반환하고,
  `import-list` 라우트는 이걸 "폴더 아님"(`isPlacelist:false`, 정상 200 응답)으로
  응답해버린다. res.ok 체크로는 절대 못 잡는 종류의 실패 - 요청 자체는 "성공"했고
  그 안의 판정 결과만 (일시적 오류 때문에) 틀렸을 뿐이라서. 클라이언트는 이걸
  진짜 "지원 안 하는 링크"와 구분할 방법이 없어 조용히 단일 링크 파싱으로
  폴백했다가 그것도 실패하면서 빈 보드로 넘어감
- **수정**: `detectPlacelist`/폴더 파서 호출/`parseUrl`(단일 링크) 전부 실패 시
  1회 재시도 추가(`import-list`/`parse` 라우트). 완전히 없앨 순 없는 종류의
  실패(외부 서비스 의존)지만, 순간적인 실패는 재시도로 대부분 넘어갈 것으로
  기대. 재시도까지 다 실패하면 이제 콘솔에 로그를 남겨서(호스트명만, URL 전체는
  안 남김) 다음에 Vercel 로그로 원인 추적 가능하게 함
- 큰 폴더(최대 200개, 페이지당 20개 = 최대 10번 순차 API 호출)는 Vercel 기본
  함수 실행 제한(10초)에 걸릴 수 있어 `import-list` 라우트에
  `export const maxDuration = 30` 추가
- `tsc`/`eslint`/`vitest`(39개)/`build` 전부 통과. **알아둘 점**: 이 수정은 "재시도로
  일시적 실패를 줄이는" 완화책이지 원천 차단은 아니다 - 네이버/카카오/구글 쪽
  서버가 계속 느리거나 응답을 안 주면 여전히 실패할 수 있음. 다음에 또 재현되면
  Vercel 로그(`vercel logs`)에 "[import-list] 폴더 링크 판별 실패" 로그가 남아있는지
  확인할 것
